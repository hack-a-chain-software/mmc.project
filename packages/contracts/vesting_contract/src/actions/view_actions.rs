...
impl Contract {
  ...
  pub fn view_investment(&self, investment_id: String) -> HashMap<String, String> {
...
mod tests {
  ...
  #[rstest]
  fn view_schema_1(mut contract: Contract, token_account: AccountId) {
    // Asserts:
    // (1) Return the serealized struct if schema with given category exists;
  ...
  fn view_schema_2(contract: Contract, token_account: AccountId) {
    // Asserts:
    // (2) Panic if there's no schema saved with the given category;
    let context = get_context(vec![], true, 0, 0, token_account, 0, DEFAULT_GAS);
    testing_env!(context);
  ...
  #[rstest]
  fn view_all_schemas_1(mut contract: Contract, token_account: AccountId) {
    // Asserts:
    // (1) Return a Vec with all serealized structs for all schema categories;
    let context = get_context(vec![], false, 0, 0, token_account, 0, DEFAULT_GAS);
    testing_env!(context);
  ...
  fn view_investment_1(contract: Contract, token_account: AccountId) {
    // Asserts:
    // (1) Assert that requested Investment exists
    let context = get_context(vec![], true, 0, 0, token_account, 0, DEFAULT_GAS);
    testing_env!(context);
  ...
  #[rstest]
  fn view_investment_2(mut contract: Contract, token_account: AccountId) {
    // Asserts:
    ...
    let investment0 = investment::Investment {
      account: AccountId::from_str("account").unwrap(),
