...
use near_sdk::{env, near_bindgen, AccountId, BorshStorageKey, PanicOnDefault, Gas};
...
impl Contract {
  ...
  pub fn new_investment(
    ...
    let investment = Investment::new(account_id.clone(), total_value.0, date_in.map(|v| v.0));
...
mod tests {
  ...
  pub const DEFAULT_GAS: Gas = Gas(10u64.pow(18));

  #[fixture]
  pub fn contract_account() -> AccountId {
    AccountId::from_str("contract.testnet").unwrap()
  }

  #[fixture]
  pub fn token_account() -> AccountId {
    AccountId::from_str("token.testnet").unwrap()
  }
  ...
  #[fixture]
  pub fn contract(owner_account: AccountId, token_account: AccountId) -> Contract {
    Contract {
      owner: owner_account,
  ...
  pub fn get_context(
    ...
    VMContext {
      view_config: None,
      current_account_id: contract_account(), // TODO: consider changing this?
      signer_account_id: signer_id.clone(),
      signer_account_pk: PublicKey::try_from([0; 33].to_vec()).unwrap(),
  ...
  #[rstest]
  fn test_new(owner_account: AccountId, token_account: AccountId) {
    let context = get_context(vec![], false, 0, 0, owner_account.clone(), 0, DEFAULT_GAS); // vec!() -> da pra inicializar assim, tem otimizacao ( macro vec)
    testing_env!(context);

    let contract = Contract::new(owner_account.clone(), token_account.clone());
    assert_eq!(contract.owner, owner_account);
    assert_eq!(contract.token_contract, token_account);
  }

  #[rstest]
  #[should_panic(expected = "The contract is not initialized")]
  fn test_default(owner_account: AccountId) {
  ...
  fn implementation_test_calculate_avalibe_withdraw(
    ...
    let investor_name = AccountId::from_str("investor.test.near").unwrap();
