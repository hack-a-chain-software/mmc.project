...
impl ExecutionOutcome {
  ...
  pub fn into_result(self) -> Result<ValueOrReceiptId> {
    match self.status {
      ExecutionStatusView::SuccessValue(value) => Ok(ValueOrReceiptId::Value(Value::from_string(
        base64::encode(value),
      ))),
...
impl Value {
  ...
  /// requirements.
  pub fn json<T: serde::de::DeserializeOwned>(&self) -> Result<T> {
    // PATCH: wrong JSON deserialization for () type
    // READ: https://github.com/near/workspaces-rs/issues/36
    let mut buf = self.raw_bytes()?;
    if buf.len() == 0 {
      buf = [110, 117, 108, 108].to_vec();
    }
    // END PATCH
  ...
  pub fn borsh<T: borsh::BorshDeserialize>(&self) -> Result<T> {
    ...
    borsh::BorshDeserialize::try_from_slice(&buf).map_err(|e| ErrorKind::DataConversion.custom(e))
  }
